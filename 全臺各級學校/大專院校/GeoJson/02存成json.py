import pandas as pd
import re
import json
import os

os.chdir(r'C:\Users\ray5547c\Desktop\專案\全臺各級學校\大專院校\GeoJson')
# while True:
#     try:
#         v = int(input('輸入版本 v='))
#         df = pd.read_excel(rf"114學年_大專校院_v{v}.xlsx")
#         break

#     except:
#         print('輸入錯誤，請重新輸入')
#         continue

df = pd.read_excel(rf"114學年_大專校院_v2.xlsx")
true_df = df[(df['範圍圖形'] == 'Polygon') | (df['範圍圖形'] == 'MultiPolygon') ]
true_df = true_df[true_df['學校名稱']=='國立臺灣大學']
row = true_df.iloc[0]
# 儲存所有 Feature
features = []
row


latlng = []

# 找出所有經緯度欄位
latlon_keys = [col for col in row.index if col.startswith('經緯度')]

for key in latlon_keys:
    text = str(row.get(key, '')).strip()
    if not text or text.lower() == 'nan':
        continue
    # 找出欄位中所有 "經度,緯度" 配對
    coords = re.findall(r'([-\d.]+)\s*,\s*([-\d.]+)', text)
    for lng, lat in coords:
        latlng.append((float(lng), float(lat)))  # 用 tuple 方便比對重複

# 檢查總數與重複數量
total_count = len(latlng)
unique_count = len(set(latlng))
duplicate_count = total_count - unique_count

print("總經緯度數:", total_count)
print("不重複經緯度數:", unique_count)
print("重複經緯度數:", duplicate_count)




name = row['學校名稱']
shape_type = str(row['範圍圖形']).strip()  # Polygon 或 MultiPolygon

# --- 收集經緯度 ---
latlng_groups = []  # 用於 MultiPolygon（多組）
latlng = []         # 單一 Polygon 用

color = '#019858'
for i in range(1, 120000):
    key = f'經緯度{i}'
    if key not in row:
        break
    text = str(row.get(key, '')).strip()
    if not text or text.lower() == 'nan':
        break

    match = re.match(r'\s*([-\d.]+)\s*,\s*([-\d.]+)\s*', text)
    if match:
        lng = float(match.group(1))
        lat = float(match.group(2))
        latlng.append([lng, lat])

    # MultiPolygon 分段判斷（可依 note 或座標重複判斷）
    # 若遇到重複的起點，可視為一個新多邊形的開頭
    if shape_type == 'MultiPolygon' and len(latlng) > 3:
        if latlng[-1] == latlng[0]:
            latlng_groups.append(latlng)
            latlng = []

# 最後一段也補上
if latlng:
    # 確保 Polygon 閉合
    if latlng[0] != latlng[-1]:
        latlng.append(latlng[0])
    if shape_type == 'MultiPolygon':
        latlng_groups.append(latlng)

# # --- 中心點 ---
# center = [float(row['中心經度']), float(row['中心緯度'])]

# --- 建立 Feature ---
if shape_type == 'Polygon' and latlng:
    features.append({
        "type": "Feature",
        "properties": {
            "name": name,
            "stroke": color,
            "stroke-width": 5,
            "fill": "#FFFFFF",
            "fill-opacity": 0
        },
        "geometry": {
            "type": "Polygon",
            "coordinates": [latlng]
        }
    })

elif shape_type == 'MultiPolygon' and latlng_groups:
    features.append({
        "type": "Feature",
        "properties": {
            "name": name,
            "stroke": color,
            "stroke-width": 5,
            "fill": "#FFFFFF",
            "fill-opacity": 0
        },
        "geometry": {
            "type": "MultiPolygon",
            "coordinates": [[group] for group in latlng_groups]
        }
    })



len(latlng_groups)















color = '#019858'
for idx, row in true_df.iterrows():
    name = row['學校名稱']
    shape_type = str(row['範圍圖形']).strip()  # Polygon 或 MultiPolygon

    # --- 收集經緯度 ---
    latlng_groups = []  # 用於 MultiPolygon（多組）
    latlng = []         # 單一 Polygon 用

    for i in range(1, 120000):
        key = f'經緯度{i}'
        if key not in row:
            break
        text = str(row.get(key, '')).strip()
        if not text or text.lower() == 'nan':
            break

        match = re.match(r'\s*([-\d.]+)\s*,\s*([-\d.]+)\s*', text)
        if match:
            lng = float(match.group(1))
            lat = float(match.group(2))
            latlng.append([lng, lat])

        # MultiPolygon 分段判斷（可依 note 或座標重複判斷）
        # 若遇到重複的起點，可視為一個新多邊形的開頭
        if shape_type == 'MultiPolygon' and len(latlng) > 3:
            if latlng[-1] == latlng[0]:
                latlng_groups.append(latlng)
                latlng = []

    # 最後一段也補上
    if latlng:
        # 確保 Polygon 閉合
        if latlng[0] != latlng[-1]:
            latlng.append(latlng[0])
        if shape_type == 'MultiPolygon':
            latlng_groups.append(latlng)

    # # --- 中心點 ---
    # center = [float(row['中心經度']), float(row['中心緯度'])]

    # --- 建立 Feature ---
    if shape_type == 'Polygon' and latlng:
        features.append({
            "type": "Feature",
            "properties": {
                "name": name,
                "stroke": color,
                "stroke-width": 5,
                "fill": "#FFFFFF",
                "fill-opacity": 0
            },
            "geometry": {
                "type": "Polygon",
                "coordinates": [latlng]
            }
        })

    elif shape_type == 'MultiPolygon' and latlng_groups:
        features.append({
            "type": "Feature",
            "properties": {
                "name": name,
                "stroke": color,
                "stroke-width": 5,
                "fill": "#FFFFFF",
                "fill-opacity": 0
            },
            "geometry": {
                "type": "MultiPolygon",
                "coordinates": [[group] for group in latlng_groups]
            }
        })

    # # --- 中心點標記 ---
    # features.append({
    #     "type": "Feature",
    #     "properties": {
    #         "name": name,
    #         "marker-color": "#db3d57",
    #         "marker-symbol": "circle"
    #     },
    #     "geometry": {
    #         "type": "Point",
    #         "coordinates": center
    #     }
    # })

# --- 輸出整體 FeatureCollection ---
geojson_data = {
    "type": "FeatureCollection",
    "features": features
}

# 輸出成 txt 檔
with open(rf"C:\Users\ray5547c\Desktop\專案\全臺各級學校\大專院校\GeoJson\114學年_大專校院_v{v}.json", 'w', encoding="utf-8") as f:
    json.dump(geojson_data, f, ensure_ascii=False, indent=2)
print(f"✅ 已成功輸出 114學年_大專校院_v{v}.json")


