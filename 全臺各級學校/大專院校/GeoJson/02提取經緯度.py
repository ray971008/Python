...
df_index = pd.read_csv(
    r"C:\Users\ray5547c\Desktop\專案\全臺各級學校\全臺各級學校_經緯度_v2.csv",
    low_memory=False
)
df_collage = pd.read_excel(
    r"C:\Users\ray5547c\Desktop\專案\全臺各級學校\大專院校\114學年_大專校院_v1.xlsx"
)
    ...
    for base_name in base_names:
        ...
        for _, row in derived_rows.iterrows():
            for col in latlon_cols:
                value = row.get(col)
                if pd.notna(value):
                    new_col = f"{latlon_prefix}{next_index}"
                    main_row[new_col] = value
                    next_index += 1
...
df_merged.columns
df_merged.to_excel(
    r"C:\Users\ray5547c\Desktop\專案\全臺各級學校\大專院校\GeoJson\114學年_大專校院_v2.xlsx",
    index=False
)
